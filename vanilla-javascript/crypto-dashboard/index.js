console.log('js file linked');

let data = []
const API_LINK = "https://api.coingecko.com/api/v3/coins/markets?vs_currency=inr&order=market_cap_desc&per_page=100&page=1&sparkline=false"

//const API_LINK = "https://jsonplaceholder.typicode.com/photos"


let cardContainer = document.getElementById("card-container");
console.log(cardContainer);

document.addEventListener("DOMContentLoaded", function (event) {
    fetch(API_LINK).then(res => res.json()).then(res => {
        let data = res
        console.log('res,', res)

        for (let i = 0; i < data.length; i++) {
            cardContainer.innerHTML += `
            <div class="card">
                <div class="image">
                    <img class="img" src="${data[i].image}" alt="image-here"/>
                </div>
                <div class="details">
                    <div class="row">
                        <span class="name">${data[i].name}</span>
                        <span class="price">${data[i].current_price}</span>
                    </div>
                    <div class="row">
                        <span class="symbol">${data[i].symbol}</span>
                        ${data[i].market_cap_change_percentage_24h < 0 ? `<span class="percent red">${data[i].market_cap_change_percentage_24h}</span>`: `<span class="percent green">${data[i].market_cap_change_percentage_24h}</span>`}
                        
                    </div>
                </div>
            </div>
        `
        }

    })
}) 